export async function runMigrations(env) {
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS commands (command_id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id INTEGER, command_name TEXT, script TEXT, aliases TEXT, schedule TEXT, version INTEGER DEFAULT 1, execution_limit INTEGER DEFAULT 0, last_executed TEXT, created_at TEXT, FOREIGN KEY (bot_id) REFERENCES bots(bot_id), UNIQUE (bot_id, command_name));`,
